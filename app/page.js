...
/*
  Focal points:
  50% 50% = center
  50% 40% = slightly higher
  55% 45% = slightly right + higher

  These are used with object-fit: cover so the media fills
  the entire hero while keeping the important area visible.
*/
const HEROES = [
  {
    type: "video",
  ...
  {
    type: "video",
    src: "/landing/IMG_6595.mp4",
    position: "55% 43%",
    mobilePosition: "53% 42%",
...
/*
  IMPORTANT:
  Both videos and images now use the same full-screen media treatment.

  There is NO blurred background.
  There is NO contain.
  Everything uses cover.

  Each hero can have its own focal point so the crop can be tuned
  without changing the rest of the landing page.
*/
function HeroMedia({ hero, className, onError }) {
  const position = hero.position || "50% 50%";
  ...
  return (
    ...
    >
      <aside className={styles.aboutPanel}>
        ...
        <h2 id="curious-title" className={styles.aboutTitle}>
        ...
        <div className={styles.aboutSection}>
          ...
          <p>
            Offbeat is for people who want to discover new places, try
            something different and make the most of the time they have,
            without spending hours planning every little detail.
          </p>
        </div>
        ...
        <div className={styles.aboutSection}>
          ...
          <p>
            I want to build a community of people who want to explore without
            spending hours in planning and executing. Sometimes you do not need
            a whole itinerary. You just need a few hours, a little curiosity
            and a reason to step out.
          </p>
        </div>
        ...
        <div className={styles.aboutSection}>
          ...
          <p>
            I&apos;m Priyanshi, a marketer and builder who loves turning ideas
            into useful little experiences. Offbeat started from a simple
            thought: everyday life has more room for adventure than we think.
          </p>
        </div>
  ...
  return (
    <main className={styles.landing}>
      <div className={styles.landingInner}>
        ...
        {showCurious && (
          <CuriousPanel onClose={() => setShowCurious(false)} />
        )}
      </div>
    </main>
  );
    ...
    apiFetch("/api/completed", user)
      ...
      .catch((err) => {
        if (active) {
          setError(
            err?.message ||
              "Could not load completed adventures."
          );
        }
      })
      .finally(() => {
...
function Generator({ user, profile, onLogout }) {
  ...
  const generate = async () => {
    ...
    try {
      const placesData = await apiFetch(
        "/api/places",
        user,
        {
          method: "POST",
          ...
          <>
            <section className={styles.panel}>
              ...
              <p className={styles.ticketSub}>
                Give Offbeat a city, a mood and a
                little time. We will handle the
                rest.
              </p>
              ...
              <article className={styles.ticket}>
                ...
                <div className={styles.stops}>
                  ...
                  {user ? (
                    ...
                    >
                      Want to keep this adventure?
                      Sign in with Google and
                      generate it again to save it
                      to your profile.
                    </div>
                  )}
                </div>
